...
# Router Admin (akses pertama dari laptop)
router_admin = {
    "device_type": "cisco_ios",
    "ip": "192.168.6.100",   # Router Admin dari laptop
    "username": "cisco",
    "password": "cisco",
}
...
def configure_router(name, ip, commands):
    """Fungsi blocking: konfigurasi IP & CDP via nested SSH"""
    try:
        ...
        output = conn.send_command_timing(ssh_cmd)
        ...
        conn.disconnect()
        print(f"    → Konfigurasi selesai untuk {name}")

    except Exception as e:
        print(f"[!] Error pada {name}: {e}")

async def worker(sem, name, ip, commands):
    async with sem:
        await asyncio.to_thread(configure_router, name, ip, commands)

async def configure_all(max_parallel=5):
    sem = asyncio.Semaphore(max_parallel)  # batasi max 5 router sekaligus
    tasks = []

    for rname, ip in router_list.items():
        if ip in router_configs:
            tasks.append(worker(sem, rname, ip, router_configs[ip]))
        else:
            print(f"[!] Warning: {rname} ({ip}) tidak ada di router_configs")

    await asyncio.gather(*tasks)

def main():
    asyncio.run(configure_all(max_parallel=5))
